# Rails API 연습

- 본 프로젝트는 Ruby On Rails 를 사용한 API 연습 레포지토리입니다.
- '당신을 위한 맞춤형 헬스 서포트 - 오빠 오늘 뭐해요'프로젝트의 백엔드 API를 만들어 본다고 생각합니다.
- 진행에 필요한 API 내용은 아래에 정의되어 있습니다.

# 프로젝트 진행 목표
- Rails 백엔드를 사용해 서비스 코드를 완성
- 코드에 정성을 들여서 조금 더 깔끔한 코드를 짜는 것을 목표로 합니다.
- API 서비스 구현에 가장 가까운 방식으로 코드를 짜는 것을 목표로 합니다.

# 개발하며 공부한 사항 / 특이사항
- 로그인 구현은 공부를 위해 jwt를 사용하여 개발함. 이때 과정을 자세히 살펴보기 위해 devise-jwt 젬을 사용하지는 않았습니다.
  
# 프로젝트 설명
- '당신을 위한 맞춤형 헬스 서포트 - 오빠 오늘 뭐해요'
- 최근 진행한 헬스 부위 또는 운동을 입력 받아서 오늘 진행하면 좋을 운동 부위를 산출
  - 프로젝트에는 간단한 알고리즘만 구현하여 사용함
  - 가슴, 팔, 어깨, 복근, 등, 하체 6가지 부분의 운동으로 구분하여 저장 후 **최근에 안해본 운동**으로 코스를 짜서 주는 것을 목표로 함
- 운동을 진행함에 따라서 추가적인 운동을 더 할 수 있습니다.
  - 운동을 할 수록 경험치가 쌓여 레벨업을 할 수 있습니다.
  
# 모델 구조 설명
모델의 구조는 다음을 포함합니다.
## Exercise
- 운동의 종류가 작성되어 있습니다.
- id
- title
- part(enum으로 작성): 운동을 한 부위
- tier_id: 운동을 할 수 있는 레벨을 저장하고 있습니다.
- content: 운동방법이 저장된 상세 내용

## User
- 운동을 하는 주체인 사용자가 정의되어 있습니다.
- id
- email
- tier_id: 사용자가 운동을 할 수 있는 레벨을 의미합니다.
  
## Workout
- 실제 운동을 한 데이터가 기록되어 있습니다.
- id
- exercise_id
- user_id
- work_time : 운동을 한 시간
- calorie_amount: 소모된 칼로리 양

## Tier
- 운동을 함에 따라서 레벨업이 가능한 경험치를 정의하고 있습니다.
- id
- level_name: (string) 현재 레벨에서 정의된  레벨의 이름
- calorie_amount: 해당 티어가 되는 데 필요한 총 경험치(소모 칼로리)의 양

# 기타 에셋  
## /db/samples/exercise_sample.csv
- 샘플 운동의 종류가 작성되어 있습니다.
- title
- part(enum으로 작성): 운동을 한 부위
- tier_id: 운동을 할 수 있는 레벨을 저장하고 있습니다.
- content: 운동방법이 저장된 상세 내용

## /db/samples/tier_sample.csv
- 샘플 레벨과 필요 경험치를 정의하고 있습니다.
- level_name: (string) 현재 레벨에서 정의된  레벨의 이름
- calorie_amount: 해당 티어가 되는 데 필요한 경험치(소모 칼로리)의 양

# API 설명
프로젝트에서 구현된 API 의 상세 설명입니다.
## 사용자 로그인
### 입력값
POST `/api/auth/sign_in`
|이름|타입|설명|옵션|
|:---:|:---:|:---:|:---:|
|email|string|사용자 고유 계정||
|password|string|사용자 비밀번호||
사용자 계정으로 로그인합니다.

jwt 방식으로 로그인합니다.

### 출력값
|이름|타입|설명|옵션|
|:---:|:---:|:---:|:---:|
|status|string|성공시 "OK"||
|access_token|string|jwt 토큰||

jwt 토큰이 반환됩니다.

### 오류 목록
|오류코드|설명|
|:---:|:---:|
|401|사용자가 인증되지 않음 (비밀번호가 틀린 문제 등 오류)|
|500|사용자 정보를 불러올 수 없음 (데이터베이스 조회에서 문제가 발생)|

이후 모든 사용에서 헤더에 다음 정보를 포함합니다.
```json
"Header": {
  "Autorization": {AUTH_TOKEN}
}
```

## 사용자 회원가입
### 입력값
POST `/api/auth/sign_up`
|이름|타입|설명|옵션|
|:---:|:---:|:---:|:---:|
|email|string|사용자 이메일||
|password|string|사용자 비밀번호||
|password_confirmation|string|사용자 비밀번호 확인||

사용자 계정으로 회원가입합니다.

### 출력값
|이름|타입|설명|옵션|
|:---:|:---:|:---:|:---:|;
|access_token|string|jwt 토큰||

jwt 토큰이 반환됩니다.

### 오류 목록
|오류코드|설명|
|:---:|:---:|
|400|파라미터로 보낸 정보가 잘못 전송됨|
|401|사용자가 인증되지 않음 (비밀번호가 틀린 문제 등 오류)|
|500|사용자 정보를 불러올 수 없음 (데이터베이스 조회에서 문제가 발생)|

## 전체 페이지
### 입력값
GET `/api/exercises`
|이름|타입|설명|옵션|
|:---:|:---:|:---:|:---:|
|-|-|-|-|
전체 운동 목록을 불러옵니다.

### 출력값
|이름|타입|설명|옵션|
|:---:|:---:|:---:|:---:|
|exercises|array|운동의 목록||


`exercises` 는 다음과 같은 구조입니다.
|이름|타입|설명|옵션|
|:---:|:---:|:---:|:---:|
|id|integer|운동의 id||
|title|string|운동의 이름||
|part|integer|운동할 범위||


### 오류 목록
|오류코드|설명|
|:---:|:---:|
|401|사용자가 인증되지 않음|
|403|현재 사용자 권한이 운동 목록을 불러 올 수 없는 권한임|
|500|전체 운동 정보를 불러올 수 없음 (데이터베이스 조회에서 문제가 발생)|

## 상세 운동 불러오기
### 입력값
GET `/api/exercises/:id`
|이름|타입|설명|옵션|
|:---:|:---:|:---:|:---:|
|-|-|-|-|
특정 id 의 운동 정보를 불러옵니다.

### 출력값
|이름|타입|설명|옵션|
|:---:|:---:|:---:|:---:|
|id|integer|운동의 id||
|title|string|운동의 이름||
|part|integer|운동할 범위||
|content|text|운동의 상세 내용||

### 오류 목록
|오류코드|설명|
|:---:|:---:|
|400|id의 형식이 잘못 지정됨|
|401|사용자가 인증되지 않음|
|403|현재 사용자 권한이 해당 운동에 접근할 수 없음|
|404|운동의 id를 찾을 수 없음|
|500|운동 정보를 불러올 수 없음 (원인 불명의 오류)|

## 추천 운동 루틴 불러오기
### 입력값
GET `/api/recommend`
|이름|타입|설명|옵션|
|:---:|:---:|:---:|:---:|
|usre_id|integer|사용자의 id||

사용자의 입력 정보를 받아와 이에 맞는 운동 루틴을 설정합니다.
추가적인 입력값은 현재 생략합니다.

### 출력값
|이름|타입|설명|옵션|
|:---:|:---:|:---:|:---:|
|exercises|array|운동의 목록||

`exercises` 는 다음과 같은 구조입니다.
|이름|타입|설명|옵션|
|:---:|:---:|:---:|:---:|
|id|integer|운동의 id||
|title|string|운동의 이름||
|part|integer|운동할 범위||


### 오류 목록
|오류코드|설명|
|:---:|:---:|
|400|파라미터로 보낸 사용자 아이디가 잘못 전송됨|
|401|사용자가 인증되지 않음(데이터베이스에 없는 사용자임)|
|403|현재 사용자 권한이 추천을 받을 수 없음|
|500|운동 정보를 불러올 수 없음 (추천 알고리즘이 잘못 작동함)|
|503|추천 시스템을 현재 사용할 수 없음|


## 운동 실행 결과 입력하기
### 입력값
POST `/api/workouts`
|이름|타입|설명|옵션|
|:---:|:---:|:---:|:---:|
|user_id|integer|사용자 id||
|exercise_id|integer|운동 id||
|work_time|integer|운동한 시간||
|calorie_amount|integer|소모된 칼로리||

사용자가 운동을 완료하면 결과를 기록하고 저장합니다.

### 출력값
|이름|타입|설명|옵션|
|:---:|:---:|:---:|:---:|
|status|string|'OK'||

### 오류 목록
|오류코드|설명|
|:---:|:---:|
|400|파라미터들 중 무엇인가가 잘못 입력됨|
|401|사용자가 인증되지 않음(데이터베이스에 없는 사용자임)|
|403|현재 사용자 권한이 입력할 수 없음|
|500|데이터를 기록할 수 없음 (데이터베이스 기록 중 문제 발생)|

## 운동 실행 결과 모두 불러오기
### 입력값
GET `/api/workouts`
|이름|타입|설명|옵션|
|:---:|:---:|:---:|:---:|
|||||

### 출력값
|이름|타입|설명|옵션|
|:---:|:---:|:---:|:---:|
|workouts|array|운동의 목록||

`workouts` 는 다음과 같은 구조입니다.
|이름|타입|설명|옵션|
|:---:|:---:|:---:|:---:|
|id|integer|운동의 id||
|exercise_id|integer|한 운동의 id||
|user_id|integer|운동한 사람의 id||
|work_time|integer|운동 수행 시간(초)||
|calorie_amount|integer|운동에 사용한 총 칼로리||


### 오류 목록
|오류코드|설명|
|:---:|:---:|
|401|사용자가 인증되지 않음(데이터베이스에 없는 사용자임)|
|403|현재 사용자 권한이 운동 기록을 불러 올 수 없는 권한임|
|500|전체 운동 정보를 불러올 수 없음 (데이터베이스 조회에서 문제가 발생)|

## 특정 운동 실행 결과 조회
### 입력값
GET `/api/workouts/:id`
|이름|타입|설명|옵션|
|:---:|:---:|:---:|:---:|
|||||

### 출력값
|이름|타입|설명|옵션|
|id|integer|운동의 id||
|exercise_id|integer|한 운동의 id||
|user_id|integer|운동한 사람의 id||
|work_time|integer|운동 수행 시간(초)||
|calorie_amount|integer|운동에 사용한 총 칼로리||


### 오류 목록
|오류코드|설명|
|:---:|:---:|
|400|없는 id를 조회하여 정보가 반환되지 않음|
|401|사용자가 인증되지 않음(데이터베이스에 없는 사용자임)|
|403|현재 사용자 권한이 운동 기록을 불러 올 수 없는 권한임|
|500|전체 운동 정보를 불러올 수 없음 (데이터베이스 조회에서 문제가 발생)|